...
- Each layer is a string with one keyword or two keywords (no definite or indefinite articles)
...
Examples of good sets:
  ...
  foundation: "rhythmic" | supporting: "vocal noises" | singing: "arpeggios"
  foundation: "birds singing" | supporting: "fireworks" | singing: "hums"
...
${arrangementSection}`;
	...
	const { output: generatedEntry } = await generateText({
		model: google(model),
		system: systemPrompt,
		prompt: `Generate the daily creative scenario and arrangement keywords for ${date}.`,
		output: Output.object({ schema: dailyEntrySchema }),
	});
